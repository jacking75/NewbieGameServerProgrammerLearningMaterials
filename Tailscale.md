...
## https 서버 주소 할당 및 사용
아래에 설명이 있어서 중복이겠지만 사용방법을 간단하게 정리하였다.  

### 완전한 외부 공개 (Tailscale Funnel) ← 외부에서 HTTPS 접속 원한다면 이것!
Tailscale이 제공하는 **Funnel** 기능을 사용하면, **Tailscale 계정이 없는 일반 인터넷 사용자도** HTTPS로 접속할 수 있다. Tailscale의 릴레이 서버를 통해 암호화된 터널을 만들어 주기 때문에 공유기 포트포워딩이나 방화벽 설정 없이도 동작한다.


**Tailscale Funnel 설정 방법:**   

**Step 1. Tailscale Admin 콘솔에서 HTTPS 인증서 활성화**  
Tailscale 관리 콘솔 → DNS 탭 → **Enable HTTPS** 활성화

**Step 2. 웹서버 실행**  
로컬에서 웹서버를 실행한다. 예: 포트 3000으로 실행 중인 서비스

**Step 3. Funnel 명령어 실행**  
```bash
# 로컬 포트 3000을 외부에 HTTPS로 공개
tailscale funnel 3000
```

그러면 자동으로 아래와 같은 공개 URL이 생성된다.

```
https://heungbae-1.tailnet-name.ts.net
```
  

**핵심 제약 사항:**  
- Funnel은 **포트 443, 8443, 10000**만 외부에서 수신 가능하다.
- Tailscale **v1.38.3 이상** 필요 (현재 이미지 기준 v1.90.9이므로 ✅ 충족)
- 트래픽 **대역폭 제한**이 있어 대규모 서비스보다는 개발/테스트용에 적합하다.
- **MagicDNS**가 활성화되어 있어야 한다.


### 443, 8443, 10000 포트 제한
Tailscale Funnel은 **Tailscale의 릴레이 서버**가 중간에서 트래픽을 받아서 여러분의 PC로 전달해주는 구조이다. 이때 릴레이 서버가 **외부 인터넷으로부터** 트래픽을 받을 수 있는 포트가 443, 8443, 10000으로 고정되어 있다는 뜻이다.

```
외부 사용자 브라우저
       ↓
  (443 또는 8443, 10000 포트로만 접속 가능)
       ↓
Tailscale 릴레이 서버 (Tailscale 회사 서버)
       ↓
  (암호화된 터널 - 포트 무관)
       ↓
내 PC (heungbae-1) 의 로컬 웹서버
```


**실제 사용 시나리오 예시:**  
내 PC에서 웹서버가 포트 **3000**으로 실행 중이라고 가정하면:

```bash
tailscale funnel 3000
```

이렇게 하면 외부 사용자는 아래처럼 접속한다.

$$\text{외부 사용자} \rightarrow \texttt{https://heungbae-1.ts.net} \rightarrow \text{Tailscale 릴레이 443포트} \rightarrow \text{내 PC:3000}$$

즉, **외부 사용자 입장에서는 그냥 `https://heungbae-1.ts.net` 주소로 접속하면 끝**이고, 포트 번호를 신경 쓸 필요가 없다. 브라우저에서 `https://` 는 기본적으로 443 포트를 사용하기 때문이다.

---

**만약 8443이나 10000 포트를 쓰고 싶다면:**
...
# 10000 포트로 외부에 노출
tailscale funnel --https=10000 3000
```

이 경우 외부 사용자는 `https://heungbae-1.ts.net:8443` 처럼 포트를 명시해서 접속해야 한다.
...
**왜 주소가 고정인가:**  
Tailscale Funnel의 URL은 아래 형식으로 생성된다.

$$\texttt{https://[머신이름].[tailnet-name].ts.net}$$

이 주소는 **Tailscale 릴레이 서버에 등록된 고정 도메인**이기 때문에, Tailscale을 재시작하거나 PC를 껐다 켜도 **머신 이름과 tailnet 이름이 바뀌지 않는 한 절대 변하지 않는다.**

---

**이미지 기준으로 예를 들면:**  
이미지에서 머신 이름이 `heungbae-1`, 계정이 `heungbae@xxx.com` 이므로 Funnel URL은 아래처럼 고정됩니다.

$$\texttt{https://heungbae-1.[tailnet-name].ts.net}$$

Tailscale 재시작, PC 재부팅, IP 변경(`100.108.134.91`) 등 **어떤 상황에서도 이 도메인 주소는 그대로 유지** 된다.

---

**단, 주의할 점이 하나 있다:**  
Funnel 설정 자체(`tailscale funnel 3000`)는 **기본적으로 영구 저장**되어 재시작 후에도 자동으로 살아난다. 하지만 아래 경우에는 Funnel이 끊길 수 있다.

- `tailscale funnel --reset` 명령어로 **직접 초기화**한 경우
- Tailscale을 **완전히 재설치**한 경우
- **머신 이름을 변경**한 경우
...
| 상황 | Funnel URL 변화 | Funnel 동작 여부 |
|---|---|---|
| Tailscale 재시작 | ❌ 안 바뀜 | ✅ 자동 유지 |
| PC 재부팅 | ❌ 안 바뀜 | ✅ 자동 유지 |
| 로컬 IP 변경 | ❌ 안 바뀜 | ✅ 자동 유지 |
| 머신 이름 변경 | ⚠️ 바뀜 | ⚠️ 새 URL 발급 |
| `--reset` 명령 실행 | ⚠️ 초기화 | ❌ 중단됨 |

즉 **한 번 설정해두면 그냥 쭉 같은 주소로 사용 가능**하다! 🚀



### Tailscale 프로그램은 꼭 실행해야하나(콘솔창에서)
Tailscale Funnel의 동작 구조를 다시 보면:
...
Tailscale 릴레이 서버
    ↓
(암호화된 터널) ← 이 터널은 Tailscale 프로그램이 살아있어야 유지됨
    ↓
내 PC의 Tailscale 프로그램 ← 여기가 종료되면 터널이 끊김
    ↓
로컬 웹서버 (포트 3000)
```

Tailscale 프로그램이 종료되면 **릴레이 서버와의 암호화 터널 자체가 끊겨버리기 때문에**, 릴레이 서버 입장에서 내 PC로 트래픽을 전달할 방법이 없어진다.
  
**정리하면:**

| 상태 | 외부 접근 가능 여부 |
|---|---|
| Tailscale 실행 중 + 웹서버 실행 중 | ✅ 가능 |
| Tailscale 실행 중 + 웹서버 종료 | ❌ 불가 (503 에러) |
| Tailscale 종료 + 웹서버 실행 중 | ❌ 불가 (터널 끊김) |
| Tailscale 종료 + 웹서버 종료 | ❌ 불가 |

즉, **외부에서 항상 접근 가능하게 하려면 Tailscale 프로그램과 웹서버 둘 다 항상 실행 중이어야 한다.** 서버 용도로 사용한다면 두 프로그램 모두 **시스템 서비스(자동 시작)로 등록**해두는 것을 권장한다! 🚀
